 ...
 */
function extractPref(inputGeojson, prefKey){
  var prefName = Utils.prefs[prefKey]
  var outputFile = "geojson/"+prefKey + ".geojson"
  var commands = ""
  commands+=" -i "+ inputGeojson
  commands+=" -filter 'pref == \""+prefName+"\"'"
  commands+=" -o force format=geojson " + outputFile
 ...
 */
function japanPrefsGeojson(inputGeojson){
  Object.keys(Utils.prefs).forEach(function(prefKey){
    extractPref(inputGeojson, prefKey)
  })
}
